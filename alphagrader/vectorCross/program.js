...
function pecesito(a,b){
  var m = a.split(" ");
  var n = b.split(" ");
  var s = ""+(parseInt(m[1])*parseInt(n[2])-parseInt(m[2])*parseInt(n[1]));
  s = s +" " +(parseInt(m[2])*parseInt(n[0])-parseInt(m[0])*parseInt(n[2]));
  s = s +" "+ (parseInt(m[0])*parseInt(n[1])-parseInt(m[1])*parseInt(n[0]));
  return s;
}
