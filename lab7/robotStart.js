...
var leftUpperLeg, rightUpperLeg, leftLowerLeg, rightLowerLeg
...
function drawRobot() {
	...
	leftUpperLeg = legPiece.clone();
	rightUpperLeg = legPiece.clone();

	leftLowerLeg = legPiece.clone();
	leftLowerLeg.position.y = -130;

	rightLowerLeg = legPiece.clone();
...
function animate() {
    ...
    forward.applyQuaternion(root.quaternion).normalize();
    // Moving Forward
    if (keyboard.pressed("W")) {
        root.translateOnAxis(forward, moveSpeed);
    }
    // Rotate Left
...
function moveLegs(){
	...
	cycle += 3;
  	leftLeg.rotation.x = Math.cos(cycle * (Math.PI/180));
	rightLeg.rotation.x = Math.cos(cycle * (Math.PI/180) + Math.PI);
}
